...
import type { Env, ToolResult, TrainTimetable } from '../types.js';
...
export async function searchTraTimetable(
  env: Env,
  ...
  try {
    ...
    const endpoint = `/Rail/TRA/DailyTrainTimetable/OD/${origin}/to/${destination}/${date}`;
    const data = await fetchEndpoint<TrainTimetable[]>(env, endpoint, { '$top': '20' });

    if (!data || data.length === 0) {
      return {
        content: [{ type: 'text', text: `找不到 ${date} 從 ${origin} 到 ${destination} 的台鐵班次` }],
      };
    }
    ...
    const header = `台鐵時刻表 ${date}（${origin} → ${destination}）共 ${data.length} 班次\n`;
    return { content: [{ type: 'text', text: header + lines.join('\n\n') }] };
  } catch (err) {
    return {
      content: [{ type: 'text', text: `查詢台鐵時刻表失敗: ${(err as Error).message}` }],
      isError: true,
    };
  }
}
