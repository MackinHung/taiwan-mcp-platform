...
vi.mock('../src/tools/tra-timetable.js', () => ({
  searchTraTimetable: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'tra-timetable' }] }),
}));
vi.mock('../src/tools/thsr-timetable.js', () => ({
  searchThsrTimetable: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'thsr-timetable' }] }),
}));
vi.mock('../src/tools/tra-liveboard.js', () => ({
  getTraLiveboard: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'tra-liveboard' }] }),
}));
vi.mock('../src/tools/metro.js', () => ({
  getMetroInfo: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'metro' }] }),
}));
vi.mock('../src/tools/bus-arrival.js', () => ({
  getBusArrival: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'bus-arrival' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with protocolVersion, name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-transit');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('search_tra_timetable');
      expect(names).toContain('search_thsr_timetable');
      expect(names).toContain('get_tra_liveboard');
      expect(names).toContain('get_metro_info');
      expect(names).toContain('get_bus_arrival');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'search_tra_timetable', arguments: { origin: '1000', destination: '3300' } },
      });
      expect((result.result as any).content[0].text).toBe('tra-timetable');
    });
