...
vi.mock('../src/tools/search-judgments.js', () => ({
  searchJudgmentsTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/get-judgment.js', () => ({
  getJudgmentTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/search-court.js', () => ({
  searchCourtTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/search-case-type.js', () => ({
  searchCaseTypeTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/recent-judgments.js', () => ({
  recentJudgmentsTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-judgment');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as any;
    expect(json.name).toBe('taiwan-judgment-mcp');
