...
vi.mock('../src/tools/liquefaction.js', () => ({
  queryLiquefactionPotential: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/faults.js', () => ({
  getActiveFaultsNearby: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/sensitive-areas.js', () => ({
  querySensitiveAreas: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/landslide.js', () => ({
  getLandslideAlerts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/geological-info.js', () => ({
  getGeologicalInfo: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-geology');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = await res.json() as any;
    expect(json.name).toBe('taiwan-geology-mcp');
