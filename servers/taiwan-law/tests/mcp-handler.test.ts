...
vi.mock('../src/tools/search-laws.js', () => ({
  searchLawsTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search' }] }),
}));
vi.mock('../src/tools/get-law.js', () => ({
  getLawTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'law' }] }),
}));
vi.mock('../src/tools/get-articles.js', () => ({
  getArticlesTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'articles' }] }),
}));
vi.mock('../src/tools/get-history.js', () => ({
  getHistoryTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'history' }] }),
}));
vi.mock('../src/tools/search-category.js', () => ({
  searchByCategoryTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'category' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-law');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_law_by_id');
      expect(names).toContain('get_law_articles');
      expect(names).toContain('get_law_history');
      expect(names).toContain('search_by_category');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'search_laws', arguments: { keyword: '民法' } },
      });
