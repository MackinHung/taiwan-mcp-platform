...
/**
 * search_transactions_by_area: 依縣市/行政區查詢不動產成交案件
 */
export async function searchTransactionsByArea(
  env: Env,
  ...
  try {
    ...
    const params: Record<string, string> = {
      page: '0',
      size: String(Math.min(limit * 3, 1000)),
    };
    ...
    const lines = sliced.map((r) => formatTransaction(r));

    const header = district
      ? `新北市${district}不動產成交資料（共 ${filtered.length} 筆，顯示 ${sliced.length} 筆）`
      : `新北市不動產成交資料（共 ${filtered.length} 筆，顯示 ${sliced.length} 筆）`;
...
function formatTransaction(r: TransactionRecord): string {
  ...
  const pricePerPing = ping > 0 ? Math.round(totalPrice / ping) : 0;
  ...
  const parts = [
    ...
    `建物面積: ${area > 0 ? `${area} m²（${ping} 坪）` : '未提供'}`,
    pricePerPing > 0 ? `每坪單價: ${formatPrice(pricePerPing)}/坪` : null,
    `建物型態: ${r.building_type || '未提供'}`,
    `格局: ${formatLayout(r)}`,
    `總樓層: ${r.total_floor || '未提供'}`,
    r.note ? `備註: ${r.note}` : null,
  ].filter(Boolean);
