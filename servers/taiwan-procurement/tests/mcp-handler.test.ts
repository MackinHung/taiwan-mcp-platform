...
vi.mock('../src/tools/search-tenders.js', () => ({
  searchTenders: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'tenders' }] }),
}));
vi.mock('../src/tools/tender-details.js', () => ({
  getTenderDetails: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'details' }] }),
}));
vi.mock('../src/tools/search-agency.js', () => ({
  searchByAgency: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'agency' }] }),
}));
vi.mock('../src/tools/awarded-contracts.js', () => ({
  getAwardedContracts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'awarded' }] }),
}));
vi.mock('../src/tools/recent-tenders.js', () => ({
  getRecentTenders: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'recent' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-procurement');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_tender_details');
      expect(names).toContain('search_by_agency');
      expect(names).toContain('get_awarded_contracts');
      expect(names).toContain('get_recent_tenders');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'search_tenders', arguments: { keyword: '資訊' } },
      });
      expect((result.result as any).content[0].text).toBe('tenders');
    });
