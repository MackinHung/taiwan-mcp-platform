...
vi.mock('../src/tools/vegetable-prices.js', () => ({
  getVegetablePrices: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'vegetables' }] }),
}));
vi.mock('../src/tools/fruit-prices.js', () => ({
  getFruitPrices: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'fruits' }] }),
}));
vi.mock('../src/tools/search-product.js', () => ({
  searchProductPrice: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search' }] }),
}));
vi.mock('../src/tools/market-summary.js', () => ({
  getMarketSummary: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'summary' }] }),
}));
vi.mock('../src/tools/compare-prices.js', () => ({
  comparePrices: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'compare' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-agri-price');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_vegetable_prices');
      expect(names).toContain('get_fruit_prices');
      expect(names).toContain('search_product_price');
      expect(names).toContain('get_market_summary');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_vegetable_prices', arguments: { market: '台北' } },
      });
      expect((result.result as any).content[0].text).toBe('vegetables');
    });
