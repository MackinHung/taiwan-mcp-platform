...
vi.mock('../src/tools/active-alerts.js', () => ({
  getActiveAlerts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'active' }] }),
}));
vi.mock('../src/tools/alerts-by-type.js', () => ({
  getAlertsByType: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'type' }] }),
}));
vi.mock('../src/tools/alerts-by-region.js', () => ({
  getAlertsByRegion: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'region' }] }),
}));
vi.mock('../src/tools/earthquake.js', () => ({
  getEarthquakeReports: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'earthquake' }] }),
}));
vi.mock('../src/tools/alert-history.js', () => ({
  getAlertHistory: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'history' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-disaster');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_alerts_by_type');
      expect(names).toContain('get_alerts_by_region');
      expect(names).toContain('get_earthquake_reports');
      expect(names).toContain('get_alert_history');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_active_alerts', arguments: {} },
      });
      expect((result.result as any).content[0].text).toBe('active');
    });
