...
vi.mock('../src/tools/river-quality.js', () => ({
  getRiverQuality: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'river-quality' }] }),
}));
vi.mock('../src/tools/station-data.js', () => ({
  getStationData: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'station-data' }] }),
}));
vi.mock('../src/tools/pollution-ranking.js', () => ({
  getPollutionRanking: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'pollution-ranking' }] }),
}));
vi.mock('../src/tools/search-by-parameter.js', () => ({
  searchByParameter: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search-param' }] }),
}));
vi.mock('../src/tools/water-quality-trends.js', () => ({
  getWaterQualityTrends: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'trends' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-water-quality');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_river_quality');
      expect(names).toContain('get_station_data');
      expect(names).toContain('get_pollution_ranking');
      expect(names).toContain('search_by_parameter');
      expect(names).toContain('get_water_quality_trends');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_river_quality', arguments: {} },
      });
      expect((result.result as any).content[0].text).toBe('river-quality');
    });
