...
vi.mock('../src/tools/all-reservoirs.js', () => ({
  getAllReservoirs: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'all' }] }),
}));
vi.mock('../src/tools/by-name.js', () => ({
  getReservoirByName: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'name' }] }),
}));
vi.mock('../src/tools/by-region.js', () => ({
  getReservoirByRegion: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'region' }] }),
}));
vi.mock('../src/tools/low-capacity.js', () => ({
  getLowCapacityAlerts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'low' }] }),
}));
vi.mock('../src/tools/details.js', () => ({
  getReservoirDetails: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'details' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-reservoir');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_all_reservoirs');
      expect(names).toContain('get_reservoir_by_name');
      expect(names).toContain('get_reservoir_by_region');
      expect(names).toContain('get_low_capacity_alerts');
      expect(names).toContain('get_reservoir_details');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_all_reservoirs', arguments: {} },
      });
      expect((result.result as any).content[0].text).toBe('all');
    });
