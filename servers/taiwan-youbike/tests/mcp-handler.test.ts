...
vi.mock('../src/tools/station-availability.js', () => ({
  getStationAvailability: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'availability' }] }),
}));
vi.mock('../src/tools/nearby-stations.js', () => ({
  searchNearbyStations: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'nearby' }] }),
}));
...
vi.mock('../src/tools/city-overview.js', () => ({
  getCityOverview: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'overview' }] }),
}));
vi.mock('../src/tools/low-availability.js', () => ({
  getLowAvailabilityAlerts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'alerts' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-youbike');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_station_availability');
      expect(names).toContain('search_nearby_stations');
      expect(names).toContain('search_by_district');
      expect(names).toContain('get_city_overview');
      expect(names).toContain('get_low_availability_alerts');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for get_station_availability', async () => {
      const result = await handleRpcRequest(env, {
        jsonrpc: '2.0',
        id: 3,
        method: 'tools/call',
        params: { name: 'get_station_availability', arguments: { city: 'taipei', stationName: 'test' } },
      });
      expect((result.result as any).content[0].text).toBe('availability');
    });
