...
vi.mock('../src/tools/minimum-wage.js', () => ({
  getMinimumWage: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'wage' }] }),
}));
vi.mock('../src/tools/labor-insurance.js', () => ({
  getLaborInsuranceInfo: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'insurance' }] }),
}));
vi.mock('../src/tools/pension.js', () => ({
  getPensionInfo: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'pension' }] }),
}));
vi.mock('../src/tools/wage-stats.js', () => ({
  getWageStatistics: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'stats' }] }),
}));
vi.mock('../src/tools/labor-laws.js', () => ({
  getLaborLawInfo: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'laws' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-labor');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_minimum_wage');
      expect(names).toContain('get_labor_insurance_info');
      expect(names).toContain('get_pension_info');
      expect(names).toContain('get_wage_statistics');
      expect(names).toContain('get_labor_law_info');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_minimum_wage', arguments: {} },
      });
      expect((result.result as any).content[0].text).toBe('wage');
    });
