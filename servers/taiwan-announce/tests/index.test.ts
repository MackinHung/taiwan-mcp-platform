...
vi.mock('../src/tools/list.js', () => ({
  listAnnouncementsTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/search.js', () => ({
  searchAnnouncementsTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/by-agency.js', () => ({
  getAnnouncementsByAgencyTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/by-date.js', () => ({
  getAnnouncementsByDateTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/stats.js', () => ({
  getAnnouncementStatsTool: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-announce');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as any;
    expect(json.name).toBe('taiwan-announce-mcp');
  ...
  it('POST /mcp route exists (requires Workers runtime for executionCtx)', async () => {
    ...
    const res = await app.fetch(req, env);
    // executionCtx is not available in Vitest, so we get 500
    // In Workers runtime this returns 200 via MCP SDK
    expect(res.status).toBe(500);
  });

  it('GET /mcp route exists (requires Workers runtime for executionCtx)', async () => {
