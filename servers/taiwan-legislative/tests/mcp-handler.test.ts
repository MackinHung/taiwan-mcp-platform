...
vi.mock('../src/tools/search-bills.js', () => ({
  searchBills: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'bills' }] }),
}));
vi.mock('../src/tools/bill-status.js', () => ({
  getBillStatus: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'status' }] }),
}));
vi.mock('../src/tools/legislator-votes.js', () => ({
  getLegislatorVotes: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'votes' }] }),
}));
vi.mock('../src/tools/search-meetings.js', () => ({
  searchMeetings: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'meetings' }] }),
}));
vi.mock('../src/tools/interpellations.js', () => ({
  getInterpellations: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'interpellations' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-legislative');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('search_bills');
      expect(names).toContain('get_bill_status');
      expect(names).toContain('get_legislator_votes');
      expect(names).toContain('search_meetings');
      expect(names).toContain('get_interpellations');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'search_bills', arguments: { keyword: '教育' } },
      });
      expect((result.result as any).content[0].text).toBe('bills');
    });
