...
vi.mock('../src/tools/search-parking.js', () => ({
  searchParking: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/realtime-availability.js', () => ({
  getRealtimeAvailability: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/parking-rates.js', () => ({
  getParkingRates: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/nearby-parking.js', () => ({
  searchNearbyParking: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/parking-summary.js', () => ({
  getParkingSummary: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-parking');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as any;
    expect(json.name).toBe('taiwan-parking-mcp');
