...
vi.mock('../src/tools/search-parking.js', () => ({
  searchParking: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search' }] }),
}));
vi.mock('../src/tools/realtime-availability.js', () => ({
  getRealtimeAvailability: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'realtime' }] }),
}));
vi.mock('../src/tools/parking-rates.js', () => ({
  getParkingRates: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'rates' }] }),
}));
vi.mock('../src/tools/nearby-parking.js', () => ({
  searchNearbyParking: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'nearby' }] }),
}));
vi.mock('../src/tools/parking-summary.js', () => ({
  getParkingSummary: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'summary' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-parking');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_realtime_availability');
      expect(names).toContain('get_parking_rates');
      expect(names).toContain('search_nearby_parking');
      expect(names).toContain('get_parking_summary');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'search_parking', arguments: { city: 'Taipei' } },
      });
