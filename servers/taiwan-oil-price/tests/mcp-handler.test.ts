...
vi.mock('../src/tools/current-prices.js', () => ({
  getCurrentPrices: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'current-prices' }] }),
}));
vi.mock('../src/tools/price-by-type.js', () => ({
  getPriceByType: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'price-by-type' }] }),
}));
vi.mock('../src/tools/price-history.js', () => ({
  getPriceHistory: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'price-history' }] }),
}));
vi.mock('../src/tools/price-change.js', () => ({
  getPriceChange: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'price-change' }] }),
}));
vi.mock('../src/tools/fuel-cost.js', () => ({
  calculateFuelCost: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'fuel-cost' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-oil-price');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_current_prices');
      expect(names).toContain('get_price_by_type');
      expect(names).toContain('get_price_history');
      expect(names).toContain('get_price_change');
      expect(names).toContain('calculate_fuel_cost');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_current_prices', arguments: {} },
      });
      expect((result.result as any).content[0].text).toBe('current-prices');
    });
