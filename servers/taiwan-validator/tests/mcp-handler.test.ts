...
vi.mock('../src/tools/national-id.js', () => ({
  validateNationalId: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'national-id' }] }),
}));
vi.mock('../src/tools/tax-id.js', () => ({
  validateTaxId: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'tax-id' }] }),
}));
vi.mock('../src/tools/phone.js', () => ({
  validatePhone: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'phone' }] }),
}));
vi.mock('../src/tools/bank-account.js', () => ({
  validateBankAccount: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'bank' }] }),
}));
vi.mock('../src/tools/license-plate.js', () => ({
  validateLicensePlate: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'plate' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-validator');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('validate_national_id');
      expect(names).toContain('validate_tax_id');
      expect(names).toContain('validate_phone');
      expect(names).toContain('validate_bank_account');
      expect(names).toContain('validate_license_plate');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'validate_national_id', arguments: { id: 'A123456789' } },
      });
      expect((result.result as any).content[0].text).toBe('national-id');
    });
