...
vi.mock('../src/tools/search-attractions.js', () => ({
  searchAttractions: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/attraction-details.js', () => ({
  getAttractionDetails: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/search-events.js', () => ({
  searchEvents: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/search-accommodation.js', () => ({
  searchAccommodation: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/get-trails.js', () => ({
  getTrails: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-tourism');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as any;
    expect(json.name).toBe('taiwan-tourism-mcp');
  ...
  it('POST / routes tools/call to correct handler', async () => {
    const req = makeRequest('POST', {
      jsonrpc: '2.0',
      id: 3,
      method: 'tools/call',
