...
vi.mock('../src/tools/expenditure.js', () => ({
  getExpenditureBudget: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/revenue.js', () => ({
  getRevenueBudget: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/agency-summary.js', () => ({
  getAgencyBudgetSummary: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/final-accounts.js', () => ({
  getFinalAccounts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/budget-search.js', () => ({
  searchBudget: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-budget');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as any;
    expect(json.name).toBe('taiwan-budget-mcp');
