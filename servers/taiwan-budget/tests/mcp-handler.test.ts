...
vi.mock('../src/tools/expenditure.js', () => ({
  getExpenditureBudget: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'expenditure' }] }),
}));
vi.mock('../src/tools/revenue.js', () => ({
  getRevenueBudget: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'revenue' }] }),
}));
vi.mock('../src/tools/agency-summary.js', () => ({
  getAgencyBudgetSummary: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'agency' }] }),
}));
vi.mock('../src/tools/final-accounts.js', () => ({
  getFinalAccounts: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'final' }] }),
}));
vi.mock('../src/tools/budget-search.js', () => ({
  searchBudget: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-budget');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('get_expenditure_budget');
      expect(names).toContain('get_revenue_budget');
      expect(names).toContain('get_agency_budget_summary');
      expect(names).toContain('get_final_accounts');
      expect(names).toContain('search_budget');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'get_expenditure_budget', arguments: { agency: '教育部' } },
      });
      expect((result.result as any).content[0].text).toBe('expenditure');
    });
