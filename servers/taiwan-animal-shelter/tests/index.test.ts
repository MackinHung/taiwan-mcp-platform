...
vi.mock('../src/tools/search-adoptable.js', () => ({
  searchAdoptableAnimals: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/animal-details.js', () => ({
  getAnimalDetails: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/search-shelters.js', () => ({
  searchShelters: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/shelter-stats.js', () => ({
  getShelterStats: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
vi.mock('../src/tools/recent-intakes.js', () => ({
  getRecentIntakes: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'ok' }] }),
}));
...
describe('Worker HTTP handler', () => {
  it('POST / with MCP initialize request returns MCP response', async () => {
    ...
    expect(json.result.serverInfo.name).toBe('taiwan-animal-shelter');
  });
  ...
  it('GET / returns server info', async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as any;
    expect(json.name).toBe('taiwan-animal-shelter-mcp');
  ...
  it('POST / with tools/call routes to tool handler', async () => {
    const req = makeRequest('POST', {
      jsonrpc: '2.0',
      id: 3,
      method: 'tools/call',
