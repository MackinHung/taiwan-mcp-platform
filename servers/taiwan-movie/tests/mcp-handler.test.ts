...
vi.mock('../src/tools/search-movies.js', () => ({
  searchMovies: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search-movies' }] }),
}));
vi.mock('../src/tools/search-cinemas.js', () => ({
  searchCinemas: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'search-cinemas' }] }),
}));
vi.mock('../src/tools/get-showtimes.js', () => ({
  getShowtimes: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'showtimes' }] }),
}));
vi.mock('../src/tools/movie-details.js', () => ({
  getMovieDetails: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'details' }] }),
}));
vi.mock('../src/tools/new-releases.js', () => ({
  getNewReleases: vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'new-releases' }] }),
}));
...
describe('handleRpcRequest', () => {
  describe('initialize', () => {
    it('returns server info with name, version, capabilities', async () => {
      ...
      expect((result.result as any).serverInfo.name).toBe('taiwan-movie');
  ...
  describe('tools/list', () => {
    it('returns all 5 tools with correct schemas', async () => {
      ...
      expect(names).toContain('search_cinemas');
      expect(names).toContain('get_showtimes');
      expect(names).toContain('get_movie_details');
      expect(names).toContain('get_new_releases');
  ...
  describe('tools/call', () => {
    it('routes to correct handler for valid tool name', async () => {
      const result = await handleRpcRequest(env, {
        ...
        id: 3,
        method: 'tools/call',
        params: { name: 'search_movies', arguments: { keyword: '紀錄片' } },
      });
      expect((result.result as any).content[0].text).toBe('search-movies');
    });
